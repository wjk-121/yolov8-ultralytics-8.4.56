...
def _ask(prompt: str, default: str = "", valid: list[str] | None = None) -> str:
    """获取用户输入，支持默认值和选项校验."""
    while True:
...
def print_table(headers: list[str], rows: list[list[str]], title: str = ""):
    ...
    widths = [
        max(len(h), *(len(str(c)) for c in col))
        for h, *col in zip(headers, *[(r + [""] * (len(headers) - len(r)))[: len(headers)] for r in rows])
    ]
...
_MODEL_TASKS = {
    "detect": {
        ...
        "desc": "目标检测",
        "cls": 80,
        "params": {"n": "3.2M", "s": "11.2M", "m": "25.9M", "l": "43.7M", "x": "68.2M"},
    },
    "seg": {
        "category": "分割模型",
        "desc": "实例分割",
        "cls": 80,
        "params": {"n": "3.4M", "s": "11.8M", "m": "27.3M", "l": "46.0M", "x": "71.8M"},
    },
    "pose": {
        "category": "姿态估计",
        "desc": "姿态估计",
        "cls": 17,
        "params": {"n": "3.3M", "s": "11.4M", "m": "26.4M", "l": "44.4M", "x": "69.4M"},
    },
    "cls": {
        "category": "分类模型",
        "desc": "图像分类",
        "cls": 1000,
        "params": {"n": "2.7M", "s": "6.4M", "m": "17.0M", "l": "37.5M", "x": "57.4M"},
    },
    "obb": {
        "category": "旋转检测",
        "desc": "旋转目标检测",
        "cls": 15,
        "params": {"n": "3.3M", "s": "11.5M", "m": "26.4M", "l": "44.5M", "x": "69.5M"},
    },
}
...
class App:
        ...
        while self.running:
            try:
                ...
                actions: dict[str, Callable] = {
                    ...
                    "2": self.detect_batch,
                    "3": self.detect_video,
                    "4": self.detect_camera,
                    "5": self.detect_url,
                    "6": self._switch_model,
                    "7": self._config_loop,
                    "8": self.show_history,
                    "9": self.show_help,
                    "0": self._exit,
                ...
                if self.running and choice != "0":
                    print()
                    input(f"{C.DIM}按回车键返回主菜单...{C.RESET}")
            except KeyboardInterrupt:
                print()
                warn("操作已中断")
    ...
    def _config_loop(self):
        while True:
            ...
            if c == "1":
                self._adj("confidence", "置信度阈值", _ask_float)
            elif c == "2":
                self._adj("iou_threshold", "IoU 阈值", _ask_float)
            elif c == "3":
                self._adj("image_size", "图像尺寸", lambda p, d: _ask_int(p, d, 320, 1920))
            elif c == "4":
                d = _ask("新的模型目录", self.cfg.models_dir)
                self.cfg.set("models_dir", d)
                self.cfg.scan_local()
                ok(f"模型目录已设置为: {d}")
            elif c == "5":
    ...
    def _load_model(self, name: str | None = None) -> bool:
    ...
    def _switch_model(self, name: str | None = None):
        if name is None:
    ...
    def _select_model(self, models: list[tuple[str, dict[str, Any]]]) -> str | None:
        while True:
            try:
    ...
    def _run_detect(
        self, mode: str, source, title: str, conf: float | None = None, stream: bool = False, show: bool = False
    ):
        if conf is None:
        ...
        if stream:
            ...
            print()
            ok(f"{mode}完成！")
            info(f"处理帧数: {frames} | 总检测数: {dets} | 耗时: {elapsed:.2f}s | FPS: {frames / elapsed:.1f}")
    ...
    def detect_batch(self):
        ...
        self._add_history("批量图片检测", folder, elapsed, sum(len(r.boxes) for r in results))
    ...
    def _display_results(self, results, elapsed: float):
        ...
        if self.cfg.save_results:
            print()
            info("结果已保存到 'runs/detect/predict'")
    ...
    def _display_batch_results(self, results, elapsed: float, total: int):
        ...
        rows = [
            ...
            ["平均/张", f"{td / total:.1f}"],
            ["耗时", f"{elapsed:.2f}s"],
            ["速度", f"{total / elapsed:.1f}张/s"],
        ]
        print_table(["统计项", "数值"], rows, "批量检测统计")
        for i, r in enumerate(results, 1):
            n = len(r.boxes)
            if n > 0:
                print(f"  {C.CYAN}图片{i}{C.RESET}: 检测到 {n} 个目标")
            else:
                print(f"  {C.DIM}图片{i}: 未检测到目标{C.RESET}")
        if self.cfg.save_results:
            print()
            info("结果已保存到 'runs/detect/predict'")
    ...
    def show_history(self):
        ...
        for i, r in enumerate(self.history, 1):
            ...
            rows.append([str(i), r["type"], src, str(r["detections"]), f"{r['elapsed_time']:.2f}s", r["timestamp"]])
